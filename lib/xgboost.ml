...
module Matrix = struct
    ...
    let from_mat = foreign ~from
        "XGDMatrixCreateFromMat" (ptr float @->
                                  ulong @-> ulong @->
                                  float @-> ptr t @-> returning int)

    let from_file = foreign ~from "XGDMatrixCreateFromFile" (string @->
        int @-> ptr t @-> returning int)

    let num_rows = foreign ~from "XGDMatrixNumRow" (t @-> ptr ulong @-> returning int)
